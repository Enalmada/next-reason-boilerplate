...
let config =
  nextSeoConfig(
    ~canonical="https://www.example.com/mobile/antdMobile",
...
let nextLink = (~todo=false, component: string) =>
  <Next.Link href={"/mobile/styleguide/" ++ component}>
    <a>
      ...
      {todo
         ? <span style={ReactDOMRe.Style.make(~fontSize="10px", ())}>
        ...
        <Layout
          ...
          <Layout.Sider
            ...
            <Menu
              ...
              <Menu.SubMenu
                key="sub1"
                title={<span> {ReasonReact.string("Layout")} </span>}>
                <Menu.Item key="Flex"> {nextLink("Flex")} </Menu.Item>
                <Menu.Item key="WingBlank">
                  {nextLink("WingBlank")}
                </Menu.Item>
                <Menu.Item key="WhiteSpace">
                  {nextLink("WhiteSpace")}
                </Menu.Item>
              </Menu.SubMenu>
              <Menu.SubMenu
                key="sub2"
                title={<span> {ReasonReact.string("Navigation")} </span>}>
                ...
                <Menu.Item key="SegmentedControl">
                  {nextLink("SegmentedControl")}
                </Menu.Item>
              ...
              <Menu.SubMenu
                key="sub3"
                title={<span> {ReasonReact.string("Data Entry")} </span>}>
                <Menu.Item key="Button"> {nextLink("Button")} </Menu.Item>
                <Menu.Item key="Calendar">
                  {nextLink("Calendar", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="Checkbox"> {nextLink("Checkbox")} </Menu.Item>
                <Menu.Item key="DatePicker">
                  {nextLink("DatePicker", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="DatePickerView">
                  {nextLink("DatePickerView", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="ImagePicker">
                  {nextLink("ImagePicker", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="InputItem">
                  {nextLink("InputItem", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="PickerView">
                  {nextLink("PickerView", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="Picker"> {nextLink("Picker")} </Menu.Item>
                <Menu.Item key="Radio"> {nextLink("Radio")} </Menu.Item>
                <Menu.Item key="Range">
                  {nextLink("Range", ~todo=true)}
                </Menu.Item>
                ...
                <Menu.Item key="Slider">
                  {nextLink("Slider", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="Stepper">
                  {nextLink("Stepper", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="TextareaItem">
                  {nextLink("TextareaItem")}
                </Menu.Item>
              </Menu.SubMenu>
              <Menu.SubMenu
                key="sub4"
                title={<span> {ReasonReact.string("Data Display")} </span>}>
                <Menu.Item key="Accordian">
                  {nextLink("Accordian", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="Badge"> {nextLink("Badge")} </Menu.Item>
                <Menu.Item key="Carousel">
                  {nextLink("Carousel", ~todo=true)}
                </Menu.Item>
                ...
                <Menu.Item key="NoticeBar">
                  {nextLink("NoticeBar")}
                </Menu.Item>
                <Menu.Item key="Steps">
                  {nextLink("Steps", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="Tag">
                  {nextLink("Tag", ~todo=true)}
                </Menu.Item>
              </Menu.SubMenu>
              <Menu.SubMenu
                key="sub5"
                title={<span> {ReasonReact.string("Feedback")} </span>}>
                <Menu.Item key="ActionSheet">
                  {nextLink("ActionSheet", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="ActivityIndicator">
                  {nextLink("ActivityIndicator")}
                </Menu.Item>
                <Menu.Item key="Modal"> {nextLink("Modal")} </Menu.Item>
                <Menu.Item key="Progress">
                  {nextLink("Progress", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="Toast"> {nextLink("Toast")} </Menu.Item>
              </Menu.SubMenu>
              <Menu.SubMenu
                key="sub6"
                title={<span> {ReasonReact.string("Gesture")} </span>}>
                <Menu.Item key="PullToRefresh">
                  {nextLink("PullToRefresh", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="SwipeAction">
                  {nextLink("SwipeAction", ~todo=true)}
                </Menu.Item>
              </Menu.SubMenu>
              <Menu.SubMenu
                key="sub7"
                title={<span> {ReasonReact.string("Combination")} </span>}>
                <Menu.Item key="ListView">
                  {nextLink("ListView", ~todo=true)}
                </Menu.Item>
                <Menu.Item key="Result">
                  {nextLink("Result", ~todo=true)}
                </Menu.Item>
              </Menu.SubMenu>
            </Menu>
          </Layout.Sider>
